  ...
  return (
    <nav className="px-8 py-10 w-full flex items-center relative z-20 md:px-12">
      ...
      <div className='right-12 fixed flex items-center'>
        <span className='text-red-600 font-medium hidden text-sm mr-3 hover:text-white md:inline transform duration-700 cursor-pointer'>MENU</span>
    ...
    <div className='w-full relative flex flex-col items-center flex-grow justify-center xs:items-start xs:px-8 sm:justify-start sm:pt-24 sm:px-12 lg:container lg:mx-auto lg:px-36'>
      ...
      <div className='flex flex-col space-y-3 mt-14 sm:space-y-0 sm:flex-row sm:space-x-8'>
        {buttons.map((name, index) => (
          <button key={index + 'button'} className={`rounded-sm py-3 px-10 sm:px-6 bg-transparent border-2 text-white border-white hover:bg-white hover:text-black transition duration-300`}>
            {name}
          </button>
        ))}
      </div>
      <ul className='absolute hidden xs:inline-block inset-y-24 right-6 lg:right-0 space-y-3'>
        {icons.map((config, index) => (
          <li key={index + 'icon'}>
            <FontAwesomeIcon icon={config.icon} inverse className="w-4 h-4 p-2 rounded-full md:border-2 border-white hover:bg-white hover:text-black cursor-pointer transition duration-300" />
          </li>
        ))}
      </ul>
    </div>
